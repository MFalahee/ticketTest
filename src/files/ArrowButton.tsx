...
//   populate the SHOWS arrow with a link here because it is convenient.
  if (props.index === 4) {
    return (
      <div onClick={() => window.open('http://www.elephantemusic.com/tour')} className="cas-square-icon-container" id={`squares-${props.index}`}>
        {temp.map((e, i) => {
          return (
            <SquareIcon key={i} className={`cas-square-${i} cas-square`} />
          );
        })}
        <PlayArrowIcon
          className={`cas-arrow`}
